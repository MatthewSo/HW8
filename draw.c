...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  for (; i < polygons->lastcol-2; i+=3){
    normal = (((polygons->m[0][i + 1] - polygons->m[0][i])* (polygons->m[1][i + 2] - polygons->m[1][i])) - (( polygons->m[1][i + 1] - polygons->m[1][i]) * (polygons->m[0][i + 2] - polygons->m[0][i])));
    //if its visible, draw lines
    if (normal >= 0){
...
void add_sphere( struct matrix * edges, 
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt <= longStop; longt++ ) {
      ...
      // normal = (((points->m[0][index + 1] - points->m[0][index])* (points->m[1][index + num_steps] - points->m[1][index])) - (( points->m[1][index + 1] - points->m[1][index]) * (points->m[0][index + num_steps] - points->m[0][index]))); 
      
      //     if ( normal > 0){ 

      if( index + num_steps > num_steps * num_steps - num_steps){
	add_polygon( edges, points->m[0][index],points->m[1][index], points->m[2][index], points->m[0][index+1], points->m[1][index+1], points->m[2][index+1], points->m[0][index % num_steps], points->m[1][index % num_steps], points->m[2][index % num_steps]);
      }
      else{
